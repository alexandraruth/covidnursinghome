...
cms_subset = read_csv("cms_subset.csv") 
...
predict_ss_num = predict(mlr_model, data.frame(anyshortage_ppe = as.factor(input$ppe), facility_size = as.numeric(input$facilitysize), max_totstaffcases = input$staffcases))

print(round(predict_ss_num, 0))
...
For a nursing home with the selected inputs, there would be a predicted `r renderPrint({print(round(predict_ss_num, 0)})` instances of self-reported staffing shortages from May 31 - December 31 2020.
